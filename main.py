...
async def logout(request: Request, response: Response):
    ...
    try:
        ...
        log_with_context("INFO", "auth", f"User logged out: {user_email}", request)
...
async def disconnect(request: Request, response: Response):
    ...
    try:
        ...
        log_with_context("INFO", "auth", f"User disconnected (revoked tokens): {user_email}", request)
...
async def google_oauth_status(request: Request, admin: dict = Depends(require_admin_auth_session)):
    ...
    try:
        ...
        if google_configured:
            config = await ttw_manager.get_google_config()
            add_log("DEBUG", "oauth_config_loaded", f"Config loaded: {config is not None}")
