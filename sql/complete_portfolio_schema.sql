-- Complete Portfolio Database Schema with UUID Foreign Keys
-- This creates the entire database schema from scratch with proper portfolio_id relationships
...
INSERT INTO work_experience (portfolio_id, company, position, location, start_date, end_date, description, is_current, company_url, sort_order) VALUES 
((SELECT portfolio_id FROM portfolios WHERE id = 'daniel-blackburn'), 'Your Current Company', 'Software Engineer/Developer', 'Remote', '2023', NULL, 'Building innovative solutions with modern technologies including FastAPI, GraphQL, and cloud infrastructure. Leading development of automated deployment pipelines and scalable web applications.', TRUE, 'https://yourcompany.com', 1),
((SELECT portfolio_id FROM portfolios WHERE id = 'daniel-blackburn'), 'Previous Company', 'Developer', 'City, State', '2021', '2023', 'Developed and maintained web applications using modern frameworks. Collaborated with cross-functional teams to deliver high-quality software solutions.', FALSE, NULL, 2)
ON CONFLICT DO NOTHING;

-- Insert sample projects
INSERT INTO projects (portfolio_id, title, description, url, technologies, sort_order) VALUES 
((SELECT portfolio_id FROM portfolios WHERE id = 'daniel-blackburn'), 'Cloud Machine Project', 'Automated cloud infrastructure and deployment pipeline with CI/CD integration. Features include automated testing, deployment orchestration, and monitoring.', 'https://github.com/blackburnd/cloud_machine_repo', '["Python", "FastAPI", "GraphQL", "CI/CD", "PostgreSQL", "Docker"]'::jsonb, 1),
((SELECT portfolio_id FROM portfolios WHERE id = 'daniel-blackburn'), 'Portfolio API', 'Modern portfolio website backend built with FastAPI and GraphQL. Features real-time contact forms, dynamic content management, and responsive design.', NULL, '["FastAPI", "GraphQL", "PostgreSQL", "HTML/CSS", "JavaScript"]'::jsonb, 2)
ON CONFLICT DO NOTHING;

-- Insert default LinkedIn OAuth scopes
INSERT INTO linkedin_oauth_scopes (portfolio_id, scope_name, display_name, description, data_access_description, is_required, sort_order) VALUES
((SELECT portfolio_id FROM portfolios WHERE id = 'daniel-blackburn'), 'r_liteprofile', 'Basic Profile', 'Access to basic profile information', 'First name, last name, profile picture, headline', true, 1),
((SELECT portfolio_id FROM portfolios WHERE id = 'daniel-blackburn'), 'r_emailaddress', 'Email Address', 'Access to primary email address', 'Primary email address associated with LinkedIn account', true, 2),
((SELECT portfolio_id FROM portfolios WHERE id = 'daniel-blackburn'), 'r_basicprofile', 'Full Profile', 'Access to full profile information', 'Complete profile including summary, location, industry', false, 3),
((SELECT portfolio_id FROM portfolios WHERE id = 'daniel-blackburn'), 'rw_company_admin', 'Company Administration', 'Access to company pages (if admin)', 'Company page information for pages you admin', false, 4),
((SELECT portfolio_id FROM portfolios WHERE id = 'daniel-blackburn'), 'w_member_social', 'Share Content', 'Ability to share content on behalf of user', 'Post updates and share content to LinkedIn feed', false, 5)
ON CONFLICT (portfolio_id, scope_name) DO UPDATE SET
